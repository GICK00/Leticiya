            ...
            switch (element)
            {
                case "Заказы":
                    sql = $"SELECT \"ORDER_ID\" AS \"Номер заказа\", \"ORDER_STATUS\" AS \"Статус заказа\", \"ORDER_DATA\" AS \"Дата заказа\", \"CUSTOMER_SURNAME\" AS \"Фамилия заказчика\", \"CUSTOMER_NAME\" AS \"Имя заказчика\"," +
                        $"\r\n\t\t\"CUSTOMER_PATRONYMIC\" AS \"Фамилия заказчика\", \"ORDER_PRICE\" AS \"Стоимость заказа\", \"CUSTOMER_ORGANIZATION\" AS \"Организация заказчика\", \"ACCOUNTANT_SURNAME\" AS \"Фамилия бухгалтера\"" +
